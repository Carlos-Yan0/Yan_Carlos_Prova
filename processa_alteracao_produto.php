...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt->bindParam(":valor_unit", $valor_unit, PDO::PARAM_INT);
    $stmt->bindParam(":id_produto", $id_produto, PDO::PARAM_INT);
    
    if ($stmt->execute()) {
        echo "<script>alert('Produto atualizado com sucesso');window.location.href='buscar_produto.php'</script>";
    }else{
        echo "<script>alert('Erro ao atualizar o produto, tente novamente!');window.location.href='buscar_produto.php?id=$id_produto';</script>";
    }
